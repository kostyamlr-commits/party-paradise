...
const EVENT_SIGNALS = {
  ...
  bachelorette: ['bachelorette','bride','hen party','bridal'],
  bachelor: ['bachelor party','groom','stag'],
  nye: ['new year','nye','countdown','2026','2027'],
  birthday: ['birthday','bday'],
  kids: ['kids','children','child','toddler'],
  romance: ['romantic','romance','valentine','couple','date night'],
  anniversary: ['anniversary'],
  surprise: ['surprise'],
}

function tagEvents(title) {
...
export async function queryAPI(keywords, cat, page=1, existingTitles=[]) {
  ...
  return clean.map(p => ({
    ...
    event_tags: tagEvents(p.product_title || ''),
  }))
}
